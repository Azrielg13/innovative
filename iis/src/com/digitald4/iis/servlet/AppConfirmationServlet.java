...
public class AppConfirmationServlet extends ParentServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException {
		try {
			Appointment appointment = Appointment.getInstance(Obfuscator.unobfuscate(request.getParameter("obf_id")));
			request.setAttribute("appointment", appointment);
			Nurse nurse = Nurse.getInstance(Obfuscator.unobfuscate(request.getParameter("nurse_oid")));
			if (appointment == null || nurse == null || nurse != appointment.getNurse()) {
				...
				if (action.equals("confirm")) {
					appointment.setNurseConfirmRes(GenData.CONFIRMED_ACCEPTED.get(),
							DateTime.now(), request.getParameter("notes")).save();
				} else if (action.equals("decline")) {
					appointment.setNurseConfirmRes(GenData.CONFIRMED_REJECTED.get(), 
